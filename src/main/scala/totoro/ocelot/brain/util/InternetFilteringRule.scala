    ...
    try {
      ...
      ruleParts.head match {
        case "allow" | "deny" =>
          ...
          val predicates = mutable.ListBuffer[(InetAddress, String) => Boolean]()
          ruleParts.tail.foreach(f => {
            ...
            filter.head match {
              case "default" =>
                if (!value) {
                  ...
                  predicates += ((inetAddress: InetAddress, host: String) => {
                    InternetFilteringRule.defaultRules
              ...
              case "ipv4-embedded-ipv6" =>
                predicates += ((inetAddress: InetAddress, _: String) => {
                  inetAddress.isInstanceOf[Inet6Address] && InetAddresses.hasEmbeddedIPv4ClientAddress(
                    inetAddress.asInstanceOf[Inet6Address]
                  )
                })
              case "domain" =>
                val domain = filter(1)
      ...
      case t: Throwable =>
        Ocelot.log.error("Invalid Internet filteringRules rule in configuration: \"" + ruleString + "\".", t)
        _invalid = true
