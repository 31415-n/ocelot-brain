...
object DriverUpgradeInventoryController extends Item with HostAware {
  ...
  override def createEnvironment(stack: ItemStack, host: EnvironmentHost): AbstractManagedEnvironment
    with WorldInventoryAnalytics with UpgradeInventoryController.Common =
    host match {
